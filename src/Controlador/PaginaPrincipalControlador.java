...
public class PaginaPrincipalControlador implements Initializable {
    ...
    //declaramos las variables asignadas a los productos
    ...
    //variables pertenecientes a los paneles donde se encuentran distintas funcionalidades
    ...
    //variables de los contenedores de las funcionalidades
    ...
    //variables pertenecientes a la funcionalidad "carrito de compra"
    @FXML private Label lblCantidadCarrito;
    @FXML private Label lblSubtotal;
    @FXML private Label lblDescuento;
    @FXML private Label lblTotal;
    
    //variables pertenecientes a la funcionalidad "mi cuenta"
    ...
    //variable que establece si el usuario registrado está activo (es decir, si un usuario está en linea)
    private Usuario usuarioActivo;
    
    //instanciamos la clase listaproducto
    private ListaProductos listaCarrito = new ListaProductos();
    //es una lista que guarda las compras hechas por el usuario (está asociada con la clase historial compras)
    private List<HistorialCompra> historialCompras = new ArrayList<>();
    //un mapa (es un tipo de diccionario) que  indica si un producto está marcado como favorito o no.
    private Map<String, Boolean> productosEnFavoritos = new HashMap<>();
    //un mapa (es un tipo de diccionario)para controlar visualmente qué productos están marcados como favoritos en la interfaz gráfica.
    private Map<String, ImageView> iconosFavoritos = new HashMap<>();
    //un mapa (es un tipo de diccionario) que  indica si un producto está marcado en lista de deseo o no.
    private Map<String, Boolean> productosEnListaDeseos = new HashMap<>();
    //un mapa (es un tipo de diccionario) para controlar visualmente qué productos están marcados en lista de deseo en la interfaz gráfica.
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        //Inicializamos el arreglo de productos para el filtrado
    ...
    //metodo para mostrar el carrito de compra
    @FXML
    ...
    //metodo general para volver de alguna funcionalidad
    @FXML
    ...
    //metodo especifico para volver del carrito
    @FXML
    ...
    //metodo para actualizar los totales del carrito de compra
    private void actualizarTotales() {
        ...
        for (Node node : contenedorCarrito.getChildren()) {
            if (node instanceof HBox) {
                ...
                //si el producto existe toma su precio y su descuento y lo calcula
                if (producto != null) {
    ...
    private void agregarProductoAlCarrito(Producto producto) {
        ...
        //mostrar opciones, contenedor, agregar producto, actualizar los totales
    ...
    //metodos para agregar los productos individualmente al carrito
    @FXML 
    ...
    //metodo para actualizar la cantidad de elementos que hay en el carrito
    ...
    private void inicializarArregloProductos() {
        // Verificar que todas los productos existan
    ...
    //metodo que se ejecuta cuando el usuario selecciona una opción en el ComboBox de ordenamiento
    @FXML
    private void filtrarProductos(ActionEvent event) {
        ...
        try {
            ...
            //verificamos si el arreglo se inicializó correctamente
            if (productosArray == null) {
            ...
            //Actualizamos la visualización con el nuevo orden
    ...
    //metodo que ordena los productos de mayor a menor precio (precio con descuento)
    private void ordenarPorPrecioMayor() {
        //verificamos si hay elementos nulos en el arreglo
        ...
        //coontinuamos con el ordenamiento normal si no hay elementos nulos
        for (int i = 0; i < productosArray.length - 1; i++) {
            for (int j = 0; j < productosArray.length - i - 1; j++) {
                // Comparamos precios con descuento aplicado
    ...
    //metodo que ordena los productos de menor a mayor precio (precio con descuento)
    private void ordenarPorPrecioMenor() {
        ...
        //continuamos con el ordenamiento normal si no hay elementos nulos
        for (int i = 0; i < productosArray.length - 1; i++) {
            for (int j = 0; j < productosArray.length - i - 1; j++) {
    ...
    //metodo que ordena los productos por porcentaje de descuento (de mayor a menor)
    ...
    //metodo que actualiza la visualización de los productos según el nuevo orden
    ...
    //metodo que muestra el panel de opciones
    @FXML
    ...
    //metodo que permite volver del panel de opciones al panel principal
    @FXML
    ...
    //metodo que muestra el panel de favoritos 
    @FXML
    ...
    //metodo que permite volver del panel de favoritos al panel principal
    @FXML
    ...
    //metodo que permite agregar un producto a favoritos
    private void agregarProductoAFavoritos(Producto producto) {
        ...
        //Si llegamos aquí, el producto no existe en favoritos, asi que añadimos uno nuevo
        HBox contenedor = new HBox(8);
        contenedor.setAlignment(Pos.CENTER_LEFT);
        contenedor.setPadding(new Insets(10));
        contenedor.setStyle("-fx-background-color: white; -fx-border-color: lightgray;");
        ...
        //mostrar controles y contenedor
    ...
    // Método que se llama cuando se hace clic en la imagen de favorito
    @FXML
    private void imagenFavoritos(MouseEvent event) {
        ...
        if (estaEnFavoritos) {
            ...
            //Cambiamos la imagen a la original
            imgFavorito.setImage(new Image(getClass().getResourceAsStream("/Vista/Imagenes/favoritos.png")));
            // Actualizamos estado
            ...
            // Cambiar la imagen a la imagen cuando está en favoritos
    ...
    //metodo que determina qué producto corresponde a la ImageView en la que se hizo clic
    ...
    //metodo para agregar un producto a la lista de deseos
    private void agregarProductoAListaDeseos(Producto producto) {
        ...
        //evento si la persona le da al boton de añadir al carrito
        ...
        //imagen para eliminar de lista de deseos
        ImageView iconoEliminar = new ImageView(new Image(getClass().getResourceAsStream("/Vista/Imagenes/borrarindividual.png")));
        iconoEliminar.setFitWidth(20);
        iconoEliminar.setFitHeight(20);
        Button btnEliminar = new Button();
        ...
        //evento si la persona le da en eliminar de favoritos
        btnEliminar.setOnAction(e -> {
            contenedorListaDeseos.getChildren().remove(contenedor);
            //Cambiamos el estado en el mapa
            productosEnListaDeseos.put(producto.getNombre(), false);

            //Cambiamos la imagen del producto
            ImageView imgListaDeseos = iconosListaDeseos.get(producto.getNombre());
            if (imgListaDeseos != null) {
                imgListaDeseos.setImage(new Image(getClass().getResourceAsStream("/Vista/Imagenes/Listadeseos.png")));
            }
        ...
        //mostramos botones y contenedor
    ...
    //metodo que se llama para cambiar la imagen que se muestra de lista de deseos en el producto
    @FXML
    private void imagenListaDeseos(MouseEvent event) {
        ...
        if (estaEnListaDeseos) {
            ...
            // Cambiar la imagen a la original
            imgListaDeseos.setImage(new Image(getClass().getResourceAsStream("/Vista/Imagenes/Listadeseos.png")));
            // Actualizar estado
            ...
            // Cambiar la imagen a la imagen cuando está en lista de deseos
    ...
    //metodo que permite determinar qué producto corresponde a la ImageView en la que se hizo clic
    ...
    //metodo para vaciar toda la lista de deseos
    @FXML
    ...
    //metodo que permite volver del pane de la lista de deseos al panel principal
    @FXML
    ...
    //metodo para agregar todos los productos de la lista de deseo al carrito
    @FXML
    private void agregarTodosAlCarrito(ActionEvent event) {
        ...
        // Creamos una copia de los elementos para evitar problemas de concurrencia
        List<Node> elementosAEliminar = new ArrayList<>(contenedorListaDeseos.getChildren());

        // iteramos por cada elemento en la lista de deseos
    ...
    //metodo auxiliar para buscar un producto por su nombre (ayudado por el de listaproductos)
    ...
    //metodo que se ejecuta cuando el usuario le da en comprar producto (individual, solo para 1 producto)
    @FXML
    ...
    //metodos para comprar el producto seleccionado
    @FXML
    ...
    //metodo para comprar los productos(los que están dentro del carrito de compras)
    @FXML
    private void comprarCarritoCompleto(ActionEvent event) {
        try {
            double total = 0;
            //inicializamos una nueva lista con los productos dentro del carrito
            List<Producto> productosEnCarrito = new ArrayList<>();

            //recorremos el nodo para extraer los productos en el
            for (Node node : contenedorCarrito.getChildren()) {
                if (node instanceof HBox) {
                    HBox item = (HBox) node;
                    VBox infoBox = (VBox) item.getChildren().get(1);
            ...
            pagosControlador.setCallbackAgregarHistorial(() -> {
                ...
                guardarHistorialEnArchivo(usuarioActivo.getUsuario());
            });

            // pasamos el metodo para vaciar carrito
    ...
    //metodo que permite ver que usuario registrado está activo
    public void setUsuarioActivo(Usuario usuario) {
        this.usuarioActivo = usuario;
        mostrarDatosUsuario();
    }

    //metodo que permite mostrar los datos del usuario activo
    ...
    //metodo que permite mostrar el panel de cuenta
    @FXML
    ...
    //metodo que permite volver del panel de cuenta al panel principal
    @FXML
    ...
    //metodo que habilita el campo a editar en mi cuenta
    private void habilitarCampo(TextField campo) {
        campo.setEditable(true);
        campo.requestFocus();
    }

    //metodos para editar los campos en mi cuenta
    @FXML
    private void editarNombre(MouseEvent event) {
        habilitarCampo(txtNombres);
    }
    @FXML
    private void editarApellidos(MouseEvent event) {
        habilitarCampo(txtApellidos);
    }
    @FXML
    private void editarSexo(MouseEvent event) {
        habilitarCampo(txtSexo);
    }
    @FXML
    private void editarEdad(MouseEvent event) {
        habilitarCampo(txtEdad);
    }
    @FXML
    private void editarPais(MouseEvent event) {
        habilitarCampo(txtPais);
    }
    @FXML
    private void editarDireccion(MouseEvent event) {
        habilitarCampo(txtDireccion);
    }
    
    //metodo para guardar los cambios de los campos editados en mi cuenta
    @FXML
    ...
    //metodo para cancelar la edicion de los cambios y volver a los que estaban originalmente
    @FXML
    ...
    //metodo para guardar los usuarios en un archivo txt
    ...
    //metodo para abrir el panel de historial de compra
    @FXML
    ...
    //metodo para cerrar el panel de historial de compra y volver al panel principal
    @FXML
    ...
    //metodo para agregar un producto comprado al historial de compra
    private void agregarProductoAHistorial(HistorialCompra historialCompra) {
        ...
        // Agregar al contenedor del historial
        contenedorHistorialCompra.getChildren().add(contenedor);
    }

    //metodo para mostrar el historial de compra con todos los productos comprados
    ...
    //metodo para guardar el historial de compra en un archivo txt
    private void guardarHistorialEnArchivo(String usuario) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("historial_" + usuario + ".txt", true))) {
            for (HistorialCompra compra : historialCompras) {
                String linea = compra.getProducto().getNombre() + "|" +
                               compra.getPrecioCompra() + "|" +
                               compra.getProducto().getImagenUrl() + "|" +
    ...
    //metodo para cargar el historial de compra de cada usuario
    void cargarHistorialDesdeArchivo(String usuario) {
        //limpiamos el historial de compra
        historialCompras.clear();
        File archivo = new File("historial_" + usuario + ".txt");
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(archivo))) {
            ...
            while ((linea = reader.readLine()) != null) {
                ...
                if (partes.length == 4) {
                    ...
                    String imagenUrl = partes[2];
                    String fecha = partes[3];

                    Producto producto = buscarProductoPorNombre(nombre); 
                    if (producto == null) {
                        producto = new Producto(nombre, precio, 0, "", imagenUrl);
                    }

                    HistorialCompra compra = new HistorialCompra(producto, fecha, precio);
                    historialCompras.add(compra);
                }
            }
        } catch (IOException e) {
